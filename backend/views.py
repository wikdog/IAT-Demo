...
def list_images(request, image_path):
    ...
    image_path = image_path.lstrip('/').replace('media/', '')
    print("image_path:", image_path)
...
def list_results_images(request):
    results_dir = '/mnt/my_volume/Inpaint-Anything-main/samproject/results/scut_replace'  # 绝对路径
    images = []

    if os.path.exists(results_dir):
        for file_name in os.listdir(results_dir):
            if file_name.endswith(('.png', '.jpg', '.jpeg', '.gif')):  # 可根据需要调整支持的文件类型
                image_path = os.path.join('results/scut_replace', file_name)  # 使用相对路径
...
def list_videos(request):
    results_dir = '/mnt/my_volume/Inpaint-Anything-main/samproject/results'  # 绝对路径
    videos = []

    if os.path.exists(results_dir):
        for file_name in os.listdir(results_dir):
            if file_name.endswith(('.mp4', '.avi', '.mov', '.wmv')):  # 根据需要调整支持的视频文件类型
                video_path = os.path.join('/results', file_name)  # 使用相对路径
                videos.append(video_path.replace("\\", "/"))  # 确保路径使用正斜杠
    else:
        return JsonResponse({'success': False, 'error': 'Directory not found'})
